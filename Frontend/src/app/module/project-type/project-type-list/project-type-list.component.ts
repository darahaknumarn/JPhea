...
export class ProjectTypeListComponent extends AbstractGrid implements OnInit {
  searchBox = new FormControl();
  @ViewChild(MatTable, {static: false}) itemTable: MatTable<any>;

  constructor
  (
    private serviceAPI: ProjectTypeService,
    private confirmService: AppConfirmService,
    private loader: AppLoaderService
  ) {super(serviceAPI, loader, { isLoad: false }); }

  ngOnInit() {
     this.list(this.pagination, { eType: EnumerationType.PROJECTTYPE})
     this.search()
  }
  ...
  private search() {
    this.searchBox.valueChanges.pipe(debounceTime(500), distinctUntilChanged()).subscribe(value => {
        this.list(this.pagination , {name:value , eType: EnumerationType.PROJECTTYPE});   
    });
  }
  ...
  getcolumn(): AppColumn[] {
    return [
      {
        displayName: 'Project Type', dataIndex: 'name'
      },
      {
