...
@Component({
  selector: 'app-expanse-item-type-form',
  templateUrl: './expanse-item-type-form.component.html',
  styleUrls: ['./expanse-item-type-form.component.scss']
})
export class ExpanseItemTypeFormComponent implements OnInit {
  ...
  constructor(
    ...
    private service: ExpanseItemTypeService,
  ...
  createForm() {
    this.form = this.fb.group({
      ...
      isActive: [true],
      isDefault: [false],
      eType: [EnumerationType.EXPANSEITEMTYPE],
      description: ['']
    });
  }
  ...
  save() {
    ...
    if( this.recId){ // update
      this.service.update( this.recId, this.form.value) .subscribe(response => {
        ...
        this.router.navigateByUrl('/expanse-item-type');
      });
    }else{// save
